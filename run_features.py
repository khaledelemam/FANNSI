...
def run_feats (input_seq):
    ...
    # print (str(len(list(MOT.keys())))+' fixed width patterns read in '+'u_fixed_w_feat.txt');    
    ...
    # check that all arguments passed
    # if (len(sys.argv)<3):
    #     eprint ("ERROR IN CALL: usage: repeat file, motif pattern file, fasta file, output file...")
    #     sys.exit(0)
    ...
    if isDirectory:
        for mfa in os.listdir(input_seq): #mfa in sys.argv[3:]:
            ...
            # print(mol_feats)
            # parseFeaturesDict(mol_feats,"query.pkl")
    elif isFile:
        # output_file=input_seq+mfa[:-4]+"_FEAT.out.txt"
        ALN = me.Alignment()
        ...
        # print(str(len(ALN.names))+' sequences read')
